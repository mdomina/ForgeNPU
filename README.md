...
- controllo seed del `scheduler` configurabile su numero di slot, iterazioni di load/compute, descrittori minimi di memoria e clear finale, con un path di code decoupled `load/store/execute` e hazard tracking minimale nei casi dedicati;
...
- simulation wrapper Python separato dal `top_npu`, con modalita' seed `graph_mode/operator_mode` per validare compiler, descriptor flow, DMA, riuso SRAM e overlap sintetico tra DMA e compute;
- comando `doctor` per diagnosticare tool EDA e stato backend LLM.

Il generatore RTL resta seed-based come baseline, ma puo' ora affiancare una variante live LLM con override controllati di `processing_element.sv` e `systolic_tile.sv`, confrontandola automaticamente contro il seed euristico tramite lo scoring corrente. Sopra il seed hardware c'e' ora anche un primo layer di `compiler/mapping` che compila un programma `LOAD/COMPUTE/STORE` dal workload e lo persiste negli artifact e nei report, includendo shape reali per GEMM, convolution, transformer attention e sparse matmul insieme al mapping plan corrispondente. Quel mapping plan seleziona adesso anche un dataflow reale `weight_stationary` o `output_stationary`, lo traduce nel segnale RTL `output_stationary_i` per `systolic_tile`, `tile_compute_unit` e `top_npu`, e lo rende visibile nel `dataflow_profile` dei report. Per i casi `output_stationary` e' presente anche un path minimo di `preload_en_i` e `transpose_inputs_i`, usato per pre-caricare i registri del tile o ruotare i vettori sorgente senza dover riscrivere l'intero mapping del cluster. Il benchmark di regressione usa adesso anche casi workload-specifici per verificare che questi `compiled_program` siano coerenti su `transformer`, `convolution`, `sparse_linear_algebra` e sul path di fallback LLM. La verifica stress non si ferma piu' al solo `top_npu`: i manifest includono anche casi interni per `scheduler`, `cluster_control` e `cluster_interconnect`, con riepilogo dedicato nel report. Lo `scheduler` seed include inoltre un caso dedicato con code decoupled `load/store/execute`, hazard tracking minimale e queue depth osservabili, mentre `execution_report.json` espone anche metriche aggregate di overlap tra memoria e compute. In aggiunta al golden model booleano, il flow produce ora un `coverage_report.json` quantitativo con copertura di moduli, stati scheduler, bank selection, tile count e stress tag. Il cluster separa in modo esplicito control-path, interconnect, scratchpad e accumulator path del top-level, con banking, descrittori minimi, burst di writeback, flush della pipeline e backpressure multi-tile instradati dai moduli dedicati; inoltre il path di store puo' passare da un `accumulator_buffer` seed con readback scalato e hook di bias, e la shape di default del cluster con `TILE_COUNT` vengono propagati direttamente dalla tile architetturale reale senza riduzioni intermedie. Le run vengono anche archiviate come dataset locale in `runs/dataset/` o nella root del benchmark corrente, con search summary best-of-N e learning feedback EDA pronti per loop di training offline. La roadmap in `NEXT_STEPS.md` e' ora entrata nel backlog ispirato a Gemmini, partendo dalla separazione esplicita dell'accumulator path, dal controllo reale del dataflow del tile, dal nuovo path minimo di preload/transpose e da un primo scheduler decoupled osservabile nei report.
...
- `candidates/<candidate_id>/verification_vectors.json`: casi per il golden model Python, inclusi `accumulator_buffer`, stress case deterministici per `top_npu` e moduli interni;
- `candidates/<candidate_id>/coverage_report.json`: coverage quantitativa dei casi di riferimento e dei tag stress;
- `candidates/<candidate_id>/simulation_wrapper_report.json`: report del wrapper di simulazione compiler-first con casi deterministici e fuzz su mismatch, overflow, reuse e overlap `DMA+compute`;
- `candidates/<candidate_id>/execution_report.json`: trace degli stati del `scheduler`, metriche memoria/compute, `dataflow_profile` con `output_stationary/preload/transpose`, queue depth e overlap metrics dello scheduler e stima di throughput del `top_npu`;
...
2. Collegare scoring e dataset ai nuovi segnali di coverage quantitativa, non solo al pass/fail dei tool.
3. Importare shape/operatori da trace o workload reali per andare oltre il compiler sintetico workload-aware attuale.
